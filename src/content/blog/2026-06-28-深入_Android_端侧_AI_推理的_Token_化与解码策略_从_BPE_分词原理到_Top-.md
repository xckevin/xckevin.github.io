...
LLM 的输入不是原始字符串，而是一个整数序列，每个整数对应词表中的一个 Token。BPE（Byte Pair Encoding）是一种子词切分算法，Gemma、Llama 等模型的分词器普遍采用它（或与之类似的 unigram 算法）。需要澄清一个常见误解：SentencePiece 本身不是 BPE 算法，而是一个分词器实现框架，内部同时支持 BPE 和 unigram 两种子词切分算法，具体用哪种由训练时的配置决定，不能把"用了 SentencePiece"直接等同于"用了 BPE"。
...
在 Android 端侧，分词器模型（无论底层用 BPE 还是 unigram 训练）以 `.model` 文件随 APK 打包，通过 SentencePiece 这类通用分词库加载，用法很简单：
...
贪婪搜索每步都选概率最高的 token，不回溯。延迟最低，但有一个明显缺陷：一旦某步选错，后面全错。更常见的是陷入重复循环——模型在某一步选了高频但无意义的 token，后续概率分布又把它推回同一个选择。这是端侧小模型使用贪婪搜索时的通病，具体的重复发生率因模型、语言和 prompt 而差异很大，没有一个可以泛化的固定百分比，实测时建议针对自己的模型和场景单独测量。

Beam Search 维护 k 个候选序列，每步扩展所有候选，保留概率乘积最高的 k 个。理论上比贪婪搜索好，但端侧落地有两个问题：beam width=4 时推理计算量是贪婪搜索的 4 倍；beam 之间容易趋同，生成文本四平八稳，缺乏多样性。实测发现，beam width 越大，首 token 延迟增长越明显，具体涨幅因设备和模型而异，端侧做 Beam Search，性价比不高。
...
解码策略不是设完就忘的参数。同一个模型，中英文的最优 Top-P 值都可能不同——中文 token 粒度更粗、分布更集中，可以适当调低 Top-P，但不建议低于 0.5 左右这个量级：Top-P 设得过低（比如 0.05-0.1）会让候选池退化到只剩 1-2 个 token，效果接近贪婪搜索，反而丢掉了采样本该带来的多样性收益。花一个下午调参，回报远超一周的 prompt engineering。
