...
做端侧 AI 功能时，很多人以为数据不出设备，隐私问题就自动解决了。但 GDPR 对"自动化决策"的约束不关心数据存在哪里。GDPR 第 22 条只约束**完全基于自动化处理**（不含人工介入）、且会对个人产生**法律效力或类似重大影响**的决策——比如自动拒保、自动拒贷这类场景。普通的本地分类或推荐不一定落入第 22 条，但如果分类结果会直接触发对用户有实质性影响的自动化决定（比如皮肤状况分类后自动推送医疗建议或限制某些功能），就需要评估是否适用。是否落入第 22 条要看具体业务场景，不能一概而论。
...
如果该场景确实落入 GDPR 第 22 条的适用范围，第 22 条第 3 款要求"数据控制者应采取适当措施保障数据主体权利"，其中"人类介入"是最难落地的一条。即便不完全落入第 22 条，从用户体验和风险控制角度，保留复核能力也是稳妥的工程实践。用户说"我不认可这个结果"，你得拿得出复核依据。
...
`inputHash` 的设计是一个权衡，但要注意它的局限：如果盐值固定（比如直接用 APPLICATION_ID）且输入向量的取值空间不大，攻击者仍可能通过预计算彩虹表或暴力枚举反推原始输入——**固定盐的加盐哈希并不能保证不可逆**，它只是提高了反查成本，不是密码学意义上的强保护。存储原始输入向量违反最小化原则，但完全不存又无法对争议结果做复核，这里选择的是"提高反查门槛"而非"绝对不可逆"的折中方案。如果需要更强的保护，应考虑加入随机 salt（每条记录独立存储）、限制输入向量精度/维度，或改用专门的隐私保护哈希方案，并结合威胁模型评估是否够用。
...
有些场景需要把端侧推理结果上报做统计分析，但这就打破了"数据不出设备"的承诺。我的做法是明确拆分：推理结果本身留在本地，上报的只是脱敏后的统计信息——比如"模型 v2.1 今天在设备上推理了 15 次，其中 Top-1 类别分布为 A:60%, B:30%, C:10%"。这些统计信息与用户身份解耦，是降低合规风险的合理工程手段，但能否严格适用某项具体豁免条款，取决于数据处理的具体上下文（是否可被重新识别、聚合粒度、留存时长等），最终结论需要法务结合具体场景判断，不是纯技术层面能下的结论。
